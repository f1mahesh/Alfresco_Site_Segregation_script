spool step-2_user_authority_insert_c3.log;
set serveroutput on size 1000000;
set define off;
prompt --step-2_user_authority_insert_c3.sql
...
Select * from  alf_child_assoc@conn_stdb ch where child_node_id in (Select p.node_id from alf_node_properties@conn_stdb p join alf_qname@conn_stdb q on q.id=p.qname_id
where (p.STRING_VALUE like 'GROUP_'||v_sponsor||'_%_SF_C%' OR p.STRING_VALUE like 'GROUP_'||v_sponsor||'_%_CLD_C%' OR p.STRING_VALUE like 'GROUP_'||v_sponsor||'_SF_C%')
