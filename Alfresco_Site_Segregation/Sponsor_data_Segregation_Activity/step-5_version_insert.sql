spool step-5_version_insert.log;
set serveroutput on size 1000000;
set define off;
prompt --step-5_version_insert.sql
...
t_num NUMBER :=0;
cursor c1 is
Select * from (Select LEVEL,ch.* from alf_child_assoc@conn_stdb ch  START WITH  ch.parent_node_id in (Select root_node_id from alf_store@conn_stdb where protocol='workspace' and IDENTIFIER='version2Store')
...
where tp.node_id in (Select id from alf_node@conn_stdb where uuid in
(Select p.string_value from alf_node_properties@conn_stdb p join alf_qname@conn_stdb q on q.id=p.qname_id where 1=1
